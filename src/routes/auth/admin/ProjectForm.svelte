<script lang="ts">
    ...
    function addCaption() {
        form.form.captions = [...form.form.captions, {tag_name: "", display_name: ""}]
    }
...
<div class="flex items-center gap-2 border-l-4 border-gray-500 bg-gray-100 p-2 text-sm">
  <span class="w-1/4 self-start p-1 font-bold text-black flex flex-col text-xs">
    ...
    <span class="text-gray-500 font-normal">Display name of the project.</span>
  </span>
  <input type="text" class="w-full bg-gray-50 p-1" bind:value={form.form.human_name} />
</div>
<div class="flex items-center gap-2 border-l-4 border-gray-500 bg-gray-100 p-2 text-sm">
  <span class="w-1/4 self-start p-1 font-bold text-black flex flex-col text-xs">
    <span class="text-sm">Captions</span>
    <span class="text-gray-500 font-normal">Set the captions to be displayed under a solution.</span>
  </span>
  <div class="flex w-full flex-col gap-1">
    {#each form.form.captions as caption, index}
    <span class="flex gap-1">
      <select class="w-full bg-gray-50 p-1" bind:value={caption.tag_name} placeholder="Attribute">
      ...
      <input type="text" class="w-full bg-gray-50 p-1" bind:value={caption.display_name} placeholder="Display Name" />
      <button type="button" class="bg-gray-200 w-fit px-3" on:click={removeCaption(index)}>-</button>
    </span>
    {/each}
    <button type="button" class="bg-gray-200 w-fit px-3" on:click={addCaption}>+</button>
  </div>
</div>
<div class="flex items-center gap-2 border-l-4 border-gray-500 bg-gray-100 p-2 text-sm">
  <span class="w-1/4 self-start p-1 font-bold text-black flex flex-col text-xs">
    <span class="text-sm">Variable Metadata</span>
    <span class="text-gray-500 font-normal">Set the measurement unit of each input parameter <br/>(or leave it blank if it's unitless)</span>
  </span>
  <div class="flex w-full flex-col gap-1">
    {#each form.form.vmetadata as meta}
    <span class="flex items-center gap-1">
      <span class="w-full">{meta.field_name}</span>
      <input type="text" class="w-full bg-gray-50 p-1" bind:value={meta.field_unit} placeholder="Unit" />
    </span>
    {/each}
  </div>
</div>
<div class="flex items-center gap-2 border-l-4 border-gray-500 bg-gray-100 p-2 text-sm">
  <span class="w-1/4 self-start p-1 font-bold text-black flex flex-col text-xs">
    ...
    <span class="text-gray-500 font-normal">Set the measurement unit of each output parameter <br/>(or leave it blank if it's unitless).</span>
  </span>
  <div class="flex w-full flex-col gap-1">
...
<div class="flex items-center gap-2 border-l-4 border-gray-500 bg-gray-100 p-2 text-sm">
  <span class="w-1/4 self-start p-1 font-bold text-black flex flex-col text-xs">
    ...
    <span class="text-gray-500 font-normal">Set the description of each asset.</span>
  </span>
  <div class="flex w-full flex-col gap-1">
    {#each form.form.ametadata as asset}
    <span class="flex items-center gap-1">
      <span class="w-full">{asset.tag}</span>
      <input type="text" class="w-full bg-gray-50 p-1" placeholder="Description" bind:value={asset.description} />
    </span>
    {/each}
  </div>
</div>
