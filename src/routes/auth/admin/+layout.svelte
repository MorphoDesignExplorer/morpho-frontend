  ...
  <div class="flex flex-col bg-blue-50 w-1/5 min-h-full pt-10 mr-10 gap-5 border-r-2 border-r-blue-500 pl-4">
    ...
    <div class="flex flex-col gap-2">
        <span class="flex justify-between items-center pr-8">
            ...
            <a href="#" class="bg-green-800 text-white font-bold py-1 px-2 text-sm">Add +</a>

        </span>

        <ol class="list-dashed-arrows ml-8">
        {#each data.projects as project}
            <li class="text-blue-600"><a href="/auth/admin/project/{project.project_name}/">{project.metadata.human_name}</a></li>
        {/each}
        </ol>
    </div>
    ...
    <div class="flex flex-col gap-2">
        <span class="flex justify-between items-center pr-8">
            ...
            <a href="/auth/admin/document/new" class="bg-green-800 text-white font-bold py-1 px-2 text-sm">Add +</a>
        </span>
    
        <ol class="list-dashed-arrows ml-8">
        {#each data.documents as document}
            <li class="text-blue-600"><a href="/auth/admin/document/{document.slug}/" target="_self">{document.slug}</a></li>
        {/each}
        </ol>
    </div>
