<script lang="ts">
    ...
    let handleDelete = async () => {
        modal.raise(`Are you sure you want to delete '${data.document.slug}'?`, async () => {
...
<form class="flex min-h-full w-1/2 flex-col gap-3 pt-10" on:submit={handleSubmit} action="?/update" method="POST" bind:this={formElement}>
    ...
    <div class="flex gap-2">
        ...
        <button class="self-start bg-green-700 font-bold text-white text-sm px-3 py-1" type="button" on:click={handleDelete}>Delete</button>
    </div>
